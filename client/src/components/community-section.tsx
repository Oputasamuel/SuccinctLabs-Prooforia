...
import { Users, Shield, ExternalLink, Clock, RefreshCw, Upload, MessageCircle, Github, CheckCircle, Copy } from "lucide-react";
...
export default function CommunitySection({ onNavigateToUpload }: CommunitySectionProps = {}) {
  const [activeTab, setActiveTab] = useState<"gallery" | "proofs" | "myproofs" | "learn" | "contribute">("gallery");
...
// My Proofs Tab Component
function MyProofsTab() {
  const { user } = useAuth();
  ...
  if (!user) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600">Please log in to view your ZK proofs.</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="text-center mb-8">
        <h3 className="text-2xl font-bold text-gray-900 mb-4">My ZK Proofs</h3>
        <p className="text-gray-600 mb-2">
          All zero-knowledge proofs generated by your wallet for minting and purchasing NFTs
        </p>
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 mb-4">
          <div className="flex items-center gap-2 text-amber-800">
            <Shield className="w-4 h-4" />
